            ...
            try {
                do {
                    ...
                    if (fd().isInputShutdown()) {
                        ...
                        //
                        // We only need to do this in the epoll transport (and not the NIO) as the NIO
                        // SocketChannelImpl.read(...) method will automatically return 0 when it was closed
                        // and so not even try to access the JNI layer.
                        break;
                    }
